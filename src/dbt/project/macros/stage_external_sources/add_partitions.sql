{% macro alter_table_add_partitions_ddl(source_node, partitions) %}
  {{ return(adapter.dispatch('alter_table_add_partitions_ddl')(source_node, partitions)) }}
{% endmacro %}
...
#}
{% macro athena__alter_table_add_partitions_ddl(source_node, partitions) %}
  ...
  {% set alter_table_add %}
    alter table {{source(source_node.source_name, source_node.name) | replace('\"', '') }} add if not exists
  {% endset %}
